...
export class LevelGenerator {
  ...
  private static generateSecretExhibitionLevel(): LevelData {
    ...
    const glowingRimMat = new THREE.MeshBasicMaterial({ color: 0x38bdf8 });
    ...
    for (let i = 0; i < regularMobs.length; i++) {
      ...
      const pedGeo = new THREE.CylinderGeometry(2.5, 2.8, 0.6, 16);
      const ped = new THREE.Mesh(pedGeo, pedestalMat);
      ped.position.set(side, 0.3, zPos);
      scene.add(ped);

      const rimGeo = new THREE.TorusGeometry(2.5, 0.08, 8, 24);
    ...
    for (let b = 0; b < bosses.length; b++) {
      ...
      const pedGeo = new THREE.CylinderGeometry(4.5, 5.0, 1.0, 24);
      const ped = new THREE.Mesh(pedGeo, pedestalMat);
      ped.position.set(xPos, 0.5, zPos);
      scene.add(ped);

      const rimGeo = new THREE.TorusGeometry(4.5, 0.12, 8, 32);
  ...
  private static generateProceduralRoomCorridorLevel(levelNumber: number): LevelData {
    ...
    for (let i = 0; i < rooms.length; i++) {
      ...
      const markerGeo = new THREE.BoxGeometry(1.6, 0.8, 0.4);
      ...
      if (room.isBossRoom) {
        ...
        const pillarMat = new THREE.MeshStandardMaterial({ color: 0x1a121c, metalness: 0.9, roughness: 0.2 });
        ...
        pOffsets.forEach(([px, pz]) => {
          ...
          const ringMesh = new THREE.Mesh(new THREE.TorusGeometry(2.3, 0.2, 8, 16), ringMat);
        ...
        const bossRingMat = new THREE.MeshStandardMaterial({ color: 0x00f0ff, emissive: 0x00f0ff, emissiveIntensity: 0.6 });
        const bossRingMesh = new THREE.Mesh(new THREE.TorusGeometry(8.0, 0.2, 8, 32), bossRingMat);
        ...
        if (isLabLevel) {
          ...
          const globeMat = new THREE.MeshBasicMaterial({ color: 0x00ffff, wireframe: true });
        ...
        if (isLabLevel) {
          ...
          if (floorId === 0) {
            ...
            if (roomVariant === 0) {
              ...
              const coreRingMat = new THREE.MeshStandardMaterial({ color: 0xffa500, emissive: 0xff5500, emissiveIntensity: 0.8 });
              const coreRing = new THREE.Mesh(new THREE.TorusGeometry(3.5, 0.15, 8, 32), coreRingMat);
      ...
      if (i < rooms.length - 1) {
        ...
        if (theme === 0) {
          ...
          const archMesh = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({ color: 0x00e1ff, emissive: 0x00b7ff, emissiveIntensity: 0.9 })
          );
    ...
    const portal = new THREE.Mesh(
      new THREE.CylinderGeometry(3.0, 3.0, 0.3, 24),
  ...
  private static decorateRoom(
    ...
    if (room.isBossRoom) {
      ...
      [-room.width / 2.5, room.width / 2.5].forEach((xOff) => {
        const beaconGeo = new THREE.CylinderGeometry(0.8, 1.2, 8.0, 8);
        const beaconMat = new THREE.MeshStandardMaterial({ color: 0x11111d, emissive: 0xff0044, emissiveIntensity: 0.6 });
    ...
    if (biomeIndex === 0) {
      ...
      const signBoard = new THREE.Mesh(
        new THREE.BoxGeometry(8.0, 3.0, 0.4),
        new THREE.MeshStandardMaterial({ color: 0x001122, emissive: 0x00f0ff, emissiveIntensity: 0.8 })
      );
  ...
  public static createSubwayBench(): THREE.Group {
    ...
    for (let b = 0; b < 2; b++) {
      const slat = new THREE.Mesh(new THREE.BoxGeometry(2.6, 0.18, 0.08), woodMat);
    ...
    // Metal Legs & Armrests
    [-1.1, 1.1].forEach((x) => {
      const leg = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.9, 0.6), metalMat);
      leg.position.set(x, 0.45, -0.1);
      leg.name = 'wall';
      group.add(leg);

      const arm = new THREE.Mesh(new THREE.BoxGeometry(0.08, 0.08, 0.6), metalMat);
  ...
  public static createSubwayTicketMachine(): THREE.Group {
    ...
    const slotMat = new THREE.MeshStandardMaterial({ color: 0x020617, metalness: 0.9 });

    // Main Cabinet Body
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.2, 2.2, 0.8), bodyMat);
    body.position.y = 1.1;
    body.name = 'wall';
    group.add(body);

    // Screen
    const screen = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.6, 0.04), screenMat);
    screen.position.set(0, 1.45, 0.41);
    group.add(screen);

    // Keypad & Slot Panel
    const panel = new THREE.Mesh(new THREE.BoxGeometry(0.9, 0.4, 0.06), slotMat);
    panel.position.set(0, 0.85, 0.41);
    group.add(panel);

    // Yellow Metro Header Bar
    const header = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.2, 0.82), yellowMat);
  ...
  public static createSubwayTurnstile(): THREE.Group {
    const group = new THREE.Group();
    const stainlessMat = new THREE.MeshStandardMaterial({ color: 0x94a3b8, metalness: 0.9, roughness: 0.2 });
    const glassMat = new THREE.MeshPhysicalMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.5, roughness: 0.1 });
    const greenLedMat = new THREE.MeshBasicMaterial({ color: 0x22c55e });

    // Cabinet Base
    const cabinet = new THREE.Mesh(new THREE.BoxGeometry(0.5, 1.1, 1.4), stainlessMat);
    cabinet.position.y = 0.55;
    cabinet.name = 'wall';
    group.add(cabinet);

    // Glass Flap Gate
    const gate = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.7, 0.8), glassMat);
    gate.position.set(0.26, 0.65, 0);
    group.add(gate);

    // Status LED Indicator
    const indicator = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 12), greenLedMat);
  ...
  public static createSubwayStationMap(): THREE.Group {
    ...
    const mapMat = new THREE.MeshBasicMaterial({ color: 0xe0f2fe });
    const line1Mat = new THREE.MeshBasicMaterial({ color: 0xef4444 });
    const line2Mat = new THREE.MeshBasicMaterial({ color: 0x3b82f6 });
    const line3Mat = new THREE.MeshBasicMaterial({ color: 0x22c55e });

    // Outer Frame
    const frame = new THREE.Mesh(new THREE.BoxGeometry(3.6, 2.0, 0.12), frameMat);
    frame.position.y = 1.0;
    frame.name = 'wall';
    group.add(frame);

    // Map Face
    const mapFace = new THREE.Mesh(new THREE.BoxGeometry(3.4, 1.8, 0.04), mapMat);
    mapFace.position.set(0, 1.0, 0.05);
    group.add(mapFace);

    // Colored Subway Line Strips
    const redLine = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.08, 0.02), line1Mat);
    redLine.position.set(0, 1.2, 0.08);
    redLine.rotation.z = 0.15;
    group.add(redLine);

    const blueLine = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.08, 0.02), line2Mat);
    blueLine.position.set(0, 0.8, 0.08);
    blueLine.rotation.z = -0.1;
    group.add(blueLine);

    const greenLine = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.08, 0.02), line3Mat);
  ...
  public static createSubwayTrainCar(): THREE.Group {
    ...
    const blueStripeMat = new THREE.MeshStandardMaterial({ color: 0x0284c7, metalness: 0.5, roughness: 0.4 });
    const darkGlassMat = new THREE.MeshPhysicalMaterial({ color: 0x0f172a, transparent: true, opacity: 0.7, roughness: 0.1 });
    const brokenGlassMat = new THREE.MeshPhysicalMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.3, roughness: 0.2 });
    const wheelMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, metalness: 0.95, roughness: 0.2 });
    const steelMat = new THREE.MeshStandardMaterial({ color: 0x94a3b8, metalness: 0.9, roughness: 0.2 });
    const seatMat = new THREE.MeshStandardMaterial({ color: 0x0369a1, roughness: 0.6 });
    const interiorFloorMat = new THREE.MeshStandardMaterial({ color: 0x020617, roughness: 0.8 });
    ...
    const emergencyLightMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });
    const rustMat = new THREE.MeshStandardMaterial({ color: 0x78350f, roughness: 0.9 });
    const debrisMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, roughness: 0.85 });
    ...
    const hull = new THREE.Mesh(new THREE.BoxGeometry(3.4, 3.2, 10.0), hullMat);
    hull.position.y = 1.8;
    hull.name = 'wall';
    group.add(hull);

    // Corrugated Roof Panel
    const roof = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.3, 9.8), roofMat);
    roof.position.y = 3.45;
    group.add(roof);

    // Roof Ribs (5 corrugated ridges along top)
    for (let r = -4.0; r <= 4.0; r += 2.0) {
      const rib = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.1, 0.2), ventMat);
    ...
    [-2.2, 2.2].forEach((z) => {
      const hvac = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.45, 1.8), ventMat);
      hvac.position.set(0, 3.8, z);
      group.add(hvac);

      // HVAC Vent Grills
      const grill = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.08, 1.4), rustMat);
      grill.position.set(0, 4.05, z);
      group.add(grill);
    });

    // Severed Pantograph Power Collector Arm
    const pantoBase = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.2, 0.8), steelMat);
    pantoBase.position.set(0, 3.7, 0);
    group.add(pantoBase);

    const pantoArm = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 1.4, 8), steelMat);
    ...
    [-1.71, 1.71].forEach((x) => {
      ...
      const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.4, 9.8), blueStripeMat);
      ...
      for (let w = -3.8; w <= 3.8; w += 1.8) {
        ...
        // Window Frame
        const frame = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.96, 1.16), steelMat);
        frame.position.set(x * 1.005, 2.2, w);
        group.add(frame);
      }

      // Jammed Sliding Doors (Center of carriage)
      const door1 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.2, 0.7), steelMat);
      door1.position.set(x * 1.01, 1.5, -0.45);
      group.add(door1);

      const door2 = new THREE.Mesh(new THREE.BoxGeometry(0.06, 2.2, 0.7), steelMat);
    ...
    // Interior Floor
    const intFloor = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.05, 9.2), interiorFloorMat);
    ...
    [-1.2, 1.2].forEach((sx) => {
      for (let sz = -3.2; sz <= 3.2; sz += 2.2) {
        ...
        const seat = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.45, 1.6), seatMat);
        seat.position.set(sx, 0.55, sz);
        group.add(seat);

        const seatBack = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.7, 1.6), seatMat);
    ...
    [-0.6, 0.6].forEach((hx) => {
      for (let hz = -3.5; hz <= 3.5; hz += 3.5) {
        const pole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 2.8, 8), steelMat);
        pole.position.set(hx, 1.7, hz);
        group.add(pole);
      }

      // Horizontal Ceiling Grab Rail
      const topRail = new THREE.Mesh(new THREE.CylinderGeometry(0.025, 0.025, 8.5, 8), steelMat);
    ...
    const interiorBeacon = new THREE.Mesh(new THREE.SphereGeometry(0.12, 12, 12), emergencyLightMat);
    ...
    const noseWin = new THREE.Mesh(new THREE.BoxGeometry(3.0, 1.2, 0.1), darkGlassMat);
    noseWin.position.set(0, 2.2, 5.01);
    noseWin.rotation.x = -0.15;
    group.add(noseWin);

    // Headlights (One working beam, one smashed/off)
    const head1 = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.1, 12), headlightMat);
    ...
    const head2 = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.1, 12), rustMat);
    ...
    [-3.2, 3.2].forEach((z) => {
      const bogie = new THREE.Mesh(new THREE.BoxGeometry(2.8, 0.5, 1.8), wheelMat);
      ...
      [-1.1, 1.1].forEach((wx) => {
        [-0.6, 0.6].forEach((wz) => {
          const wheel = new THREE.Mesh(new THREE.CylinderGeometry(0.38, 0.38, 0.12, 16), steelMat);
    ...
    for (let t = -4.5; t <= 4.5; t += 3.0) {
      const railSegment = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.12, 3.2), steelMat);
  ...
  public static createSubwayPlatformPillar(): THREE.Group {
    ...
    const tileCapMat = new THREE.MeshStandardMaterial({ color: 0x0284c7, metalness: 0.3, roughness: 0.4 });

    // Main Pillar Body
    const body = new THREE.Mesh(new THREE.BoxGeometry(1.2, 5.0, 1.2), pillarMat);
    body.position.y = 2.5;
    body.name = 'wall';
    group.add(body);

    // Blue Station Tile Accent Band
    const band = new THREE.Mesh(new THREE.BoxGeometry(1.26, 0.8, 1.26), tileCapMat);
    band.position.y = 3.5;
    group.add(band);
  ...
  public static createSubwayTrashBin(): THREE.Group {
    const group = new THREE.Group();
    const steelMat = new THREE.MeshStandardMaterial({ color: 0x64748b, metalness: 0.85, roughness: 0.3 });
    const topMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, roughness: 0.5 });

    const bin = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.35, 0.9, 16), steelMat);
    bin.position.y = 0.45;
    bin.name = 'wall';
    group.add(bin);

    const lid = new THREE.Mesh(new THREE.CylinderGeometry(0.37, 0.37, 0.1, 16), topMat);
    lid.position.y = 0.92;
    group.add(lid);
  ...
  public static createSubwayStationLamps(): THREE.Group {
    const group = new THREE.Group();
    const frameMat = new THREE.MeshStandardMaterial({ color: 0x334155, metalness: 0.8 });
    const lampMat = new THREE.MeshBasicMaterial({ color: 0xe0f2fe });

    const fixture = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.15, 0.4), frameMat);
    fixture.position.y = 0;
    group.add(fixture);

    const tube = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 2.8, 12), lampMat);
  ...
  public static createUltraDetailedServerRack(): THREE.Group {
    ...
    const frameMat = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.4, metalness: 0.75 });
    const glassDoorMat = new THREE.MeshPhysicalMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.35, roughness: 0.1 });
    ...
    const tower = new THREE.Mesh(new THREE.BoxGeometry(1.8, 3.6, 1.2), bodyMat);
    tower.position.y = 1.8;
    tower.name = 'wall';
    group.add(tower);

    // Frame Borders
    const frontFrame = new THREE.Mesh(new THREE.BoxGeometry(1.84, 3.64, 0.1), frameMat);
    frontFrame.position.set(0, 1.8, 0.58);
    group.add(frontFrame);

    // Front Glass Door
    const glassDoor = new THREE.Mesh(new THREE.BoxGeometry(1.7, 3.5, 0.04), glassDoorMat);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const blade = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.35, 1.0), frameMat);
      blade.position.set(0, bladeY, 0.05);
      group.add(blade);

      // Server LEDs
      const greenLed = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), ledGreenMat);
      greenLed.position.set(-0.7, bladeY, 0.58);
      group.add(greenLed);

      const cyanLed = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), ledCyanMat);
      cyanLed.position.set(-0.58, bladeY, 0.58);
      group.add(cyanLed);

      if (i % 3 === 0) {
        const redLed = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), ledRedMat);
    ...
    const fanGrille = new THREE.Mesh(new THREE.CylinderGeometry(0.4, 0.4, 0.08, 16), frameMat);
  ...
  public static createUltraDetailedControlConsole(): THREE.Group {
    ...
    const buttonRed = new THREE.MeshBasicMaterial({ color: 0xef4444 });
    const buttonYellow = new THREE.MeshBasicMaterial({ color: 0xf59e0b });
    const buttonGreen = new THREE.MeshBasicMaterial({ color: 0x22c55e });

    // Base Desk Unit
    const baseDesk = new THREE.Mesh(new THREE.BoxGeometry(2.4, 1.1, 1.2), bodyMat);
    baseDesk.position.y = 0.55;
    baseDesk.name = 'wall';
    group.add(baseDesk);

    // Angled Monitor Panel
    const monitorPanel = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.9, 0.2), bodyMat);
    monitorPanel.position.set(0, 1.45, -0.3);
    monitorPanel.rotation.x = -0.35;
    group.add(monitorPanel);

    // Main Display Screen
    const screen = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.75, 0.05), screenMat);
    ...
    for (let b = 0; b < 6; b++) {
      ...
      const btn = new THREE.Mesh(new THREE.BoxGeometry(0.12, 0.05, 0.12), mat);
  ...
  public static createUltraDetailedEquipmentLocker(): THREE.Group {
    ...
    const doorMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, roughness: 0.3, metalness: 0.85 });
    const lockMat = new THREE.MeshBasicMaterial({ color: 0x22c55e });

    // Main Cabinet
    const locker = new THREE.Mesh(new THREE.BoxGeometry(1.6, 2.8, 0.9), bodyMat);
    ...
    [-0.38, 0.38].forEach((xOff) => {
      const door = new THREE.Mesh(new THREE.BoxGeometry(0.72, 2.6, 0.05), doorMat);
      door.position.set(xOff, 1.4, 0.46);
      group.add(door);
    });

    // Magnetic Keypad Lock
    const lockLight = new THREE.Mesh(new THREE.SphereGeometry(0.05, 8, 8), lockMat);
  ...
  public static createUltraDetailedBioStorage(): THREE.Group {
    ...
    // Wooden Pallet Base
    const pallet = new THREE.Mesh(new THREE.BoxGeometry(2.2, 0.2, 2.2), palletMat);
  ...
  public static createUltraDetailedWorkstation(): THREE.Group {
    ...
    const darkSteelMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, metalness: 0.9, roughness: 0.2 });
    const chromeMat = new THREE.MeshStandardMaterial({ color: 0xcbd5e1, metalness: 0.95, roughness: 0.1 });
    const pcCaseMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.7, roughness: 0.3 });
    const pcbMat = new THREE.MeshStandardMaterial({ color: 0x15803d, metalness: 0.3, roughness: 0.5 }); // Green motherboard
    const screenMat = new THREE.MeshBasicMaterial({ color: 0x22c55e }); // Glowing matrix code screen
    const paperMat = new THREE.MeshStandardMaterial({ color: 0xf8fafc, roughness: 0.8 });
    const copperWireMat = new THREE.MeshStandardMaterial({ color: 0xef4444, roughness: 0.3 });

    // 1. Table Frame Assembly (12 sub-parts)
    const topPlate = new THREE.Mesh(new THREE.BoxGeometry(3.0, 0.12, 1.6), steelMat);
    topPlate.position.set(0, 0, 0);
    group.add(topPlate);

    const topBevel = new THREE.Mesh(new THREE.BoxGeometry(3.08, 0.04, 1.68), darkSteelMat);
    topBevel.position.set(0, 0.04, 0);
    group.add(topBevel);

    // Cable grommet hole
    const grommet = new THREE.Mesh(new THREE.TorusGeometry(0.08, 0.02, 8, 16), chromeMat);
    ...
    legPositions.forEach(([lx, ly, lz]) => {
      const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 1.15, 12), steelMat);
      leg.position.set(lx, ly, lz);
      group.add(leg);

      const foot = new THREE.Mesh(new THREE.CylinderGeometry(0.09, 0.09, 0.05, 12), darkSteelMat);
      foot.position.set(lx, ly - 0.58, lz);
      group.add(foot);
    });

    // Cross brace support bar under desk
    const brace = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 2.6, 8), chromeMat);
    brace.rotation.z = Math.PI / 2;
    brace.position.set(0, -0.5, -0.5);
    group.add(brace);

    // Wire tray basket under desk
    const tray = new THREE.Mesh(new THREE.BoxGeometry(2.0, 0.1, 0.3), darkSteelMat);
    tray.position.set(0, -0.15, -0.6);
    group.add(tray);

    // Coiled cable wire hanging from tray
    for (let w = 0; w < 3; w++) {
      const wire = new THREE.Mesh(new THREE.CylinderGeometry(0.015, 0.015, 0.8, 6), copperWireMat);
    ...
    const pcCase = new THREE.Mesh(new THREE.BoxGeometry(0.5, 0.7, 0.8), pcCaseMat);
    pcGroup.add(pcCase);

    // Motherboard PCB inside
    const mobo = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.55, 0.65), pcbMat);
    mobo.position.set(-0.2, 0, 0);
    pcGroup.add(mobo);

    // CPU cooler heatsink tower block
    const cooler = new THREE.Mesh(new THREE.BoxGeometry(0.18, 0.18, 0.18), chromeMat);
    cooler.position.set(-0.08, 0.1, 0);
    pcGroup.add(cooler);

    // Power supply unit box
    const psu = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.22, 0.3), darkSteelMat);
    psu.position.set(0, -0.2, -0.2);
    pcGroup.add(psu);

    // Detached side panel popped off onto desk
    const sidePanel = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.68, 0.78), chromeMat);
    ...
    const monBase = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.28, 0.04, 12), darkSteelMat);
    monGroup.add(monBase);

    const monArm = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 0.5, 8), chromeMat);
    monArm.position.set(0, 0.25, -0.1);
    monArm.rotation.x = -0.2;
    monGroup.add(monArm);

    const monBody = new THREE.Mesh(new THREE.BoxGeometry(1.1, 0.75, 0.12), pcCaseMat);
    monBody.position.set(0, 0.5, -0.15);
    monGroup.add(monBody);

    const monScreen = new THREE.Mesh(new THREE.BoxGeometry(1.0, 0.66, 0.02), screenMat);
    ...
    const kbBase = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.04, 0.3), darkSteelMat);
    kbBase.position.set(-0.4, 0.08, 0.3);
    kbBase.rotation.y = -0.1;
    group.add(kbBase);

    for (let k = 0; k < 12; k++) {
      const keycap = new THREE.Mesh(new THREE.BoxGeometry(0.035, 0.03, 0.035), chromeMat);
      keycap.position.set(
        -0.7 + Math.random() * 0.6,
    ...
    for (let p = 0; p < 5; p++) {
      const paper = new THREE.Mesh(new THREE.BoxGeometry(0.25, 0.005, 0.35), paperMat);
  ...
  public static createUltraDetailedCryoTank(): THREE.Group {
    ...
    const octMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.9, roughness: 0.2 });
    const chromeMat = new THREE.MeshStandardMaterial({ color: 0x64748b, metalness: 0.95, roughness: 0.1 });
    const warningYellowMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, roughness: 0.4 });
    ...
    const slimeMat = new THREE.MeshBasicMaterial({ color: 0x22c55e });
    const bubbleMat = new THREE.MeshBasicMaterial({ color: 0xbbf7d0 });
    const dialFaceMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });

    // 1. Heavy Octagonal Base Pedestal (8 sub-parts)
    const basePedestal = new THREE.Mesh(new THREE.CylinderGeometry(1.2, 1.4, 0.5, 8), octMat);
    basePedestal.position.set(0, 0.25, 0);
    group.add(basePedestal);

    const baseRim = new THREE.Mesh(new THREE.TorusGeometry(1.25, 0.08, 8, 16), warningYellowMat);
    ...
    for (let h = 0; h < 4; h++) {
      ...
      const piston = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.6, 12), chromeMat);
      piston.position.set(px, 0.4, pz);
      group.add(piston);
    }

    // Pressure Control Manifold with Gauge Dials
    const manifold = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.3, 0.2), octMat);
    manifold.position.set(0, 0.4, 1.25);
    group.add(manifold);

    for (let g = 0; g < 3; g++) {
      const dial = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.04, 12), dialFaceMat);
    ...
    const glassHull = new THREE.Mesh(new THREE.CylinderGeometry(0.9, 0.9, 2.2, 16, 1, true), glassMat);
    glassHull.position.set(0, 1.6, 0);
    group.add(glassHull);

    const topCap = new THREE.Mesh(new THREE.CylinderGeometry(0.95, 1.05, 0.4, 12), octMat);
    ...
    const slimePool = new THREE.Mesh(new THREE.CylinderGeometry(2.8, 2.8, 0.03, 24), slimeMat);
  ...
  public static createUltraDetailedBiohazardBarrel(): THREE.Group {
    ...
    const barrelMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, roughness: 0.45, metalness: 0.65 });
    const rimMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, metalness: 0.9, roughness: 0.2 });
    const labelMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });

    const body = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.55, 1.4, 18), barrelMat);
    group.add(body);

    [-0.4, 0, 0.4].forEach((yOff) => {
      const ring = new THREE.Mesh(new THREE.TorusGeometry(0.57, 0.03, 8, 20), rimMat);
      ring.rotation.x = Math.PI / 2;
      ring.position.set(0, yOff, 0);
      group.add(ring);
    });

    const topRim = new THREE.Mesh(new THREE.TorusGeometry(0.56, 0.04, 8, 20), rimMat);
    topRim.rotation.x = Math.PI / 2;
    topRim.position.set(0, 0.7, 0);
    group.add(topRim);

    for (let c = 0; c < 2; c++) {
      const cap = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 0.05, 10), rimMat);
      cap.position.set(-0.2 + c * 0.4, 0.72, 0);
      group.add(cap);
    }

    const label = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.3, 0.02), labelMat);
  ...
  public static createUltraDetailedCeilingFixture(): THREE.Group {
    ...
    const fixtureMat = new THREE.MeshStandardMaterial({ color: 0x334155, metalness: 0.8, roughness: 0.3 });
    const tubeMat = new THREE.MeshBasicMaterial({ color: 0xfef08a });
    const wireMat1 = new THREE.MeshStandardMaterial({ color: 0xef4444, metalness: 0.9, roughness: 0.1 });
    const wireMat2 = new THREE.MeshStandardMaterial({ color: 0x38bdf8, metalness: 0.9, roughness: 0.1 });
    const wireMat3 = new THREE.MeshStandardMaterial({ color: 0xf59e0b, metalness: 0.9, roughness: 0.1 });

    const housing = new THREE.Mesh(new THREE.BoxGeometry(2.4, 0.15, 0.8), fixtureMat);
    group.add(housing);

    const tube1 = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 2.2, 12), tubeMat);
    tube1.rotation.z = Math.PI / 2;
    tube1.position.set(0, -0.1, -0.2);
    group.add(tube1);

    const tube2 = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.04, 2.2, 12), tubeMat);
  ...
  public static createUltraDetailedWallProps(): THREE.Group {
    ...
    const redMetalMat = new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.3, metalness: 0.8 });
    const steelMat = new THREE.MeshStandardMaterial({ color: 0x475569, metalness: 0.9, roughness: 0.2 });
    const glassMat = new THREE.MeshPhysicalMaterial({ color: 0xe2e8f0, transparent: true, opacity: 0.5, roughness: 0.1 });
    const crossMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });

    const extTank = new THREE.Mesh(new THREE.CylinderGeometry(0.18, 0.18, 0.8, 16), redMetalMat);
    extTank.position.set(0, 0, 0);
    group.add(extTank);

    const extCap = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.18, 0.15, 12), steelMat);
    extCap.position.set(0, 0.45, 0);
    group.add(extCap);

    const extHandle = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.04, 0.1), steelMat);
    extHandle.position.set(0.08, 0.52, 0);
    group.add(extHandle);

    const extHose = new THREE.Mesh(new THREE.TorusGeometry(0.15, 0.02, 8, 12, Math.PI), steelMat);
    extHose.rotation.y = Math.PI / 2;
    extHose.position.set(0.15, 0.2, 0);
    group.add(extHose);

    const extBracket = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.4, 0.25), steelMat);
    ...
    const boxFrame = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.6, 0.5), steelMat);
    boxGroup.add(boxFrame);

    const boxGlass = new THREE.Mesh(new THREE.BoxGeometry(0.02, 0.5, 0.42), glassMat);
    boxGlass.position.set(0.08, 0, 0);
    boxGroup.add(boxGlass);

    const crossH = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.2, 0.06), crossMat);
    crossH.position.set(0.09, 0, 0);
    boxGroup.add(crossH);

    const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.03, 0.06, 0.2), crossMat);
  ...
  public static createUltraDetailedSlopeRampGroup(
    ...
    // Materials
    const darkSteelMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.85, roughness: 0.25 });
    const steelMat = new THREE.MeshStandardMaterial({ color: 0x475569, metalness: 0.9, roughness: 0.2 });
    const chromeMat = new THREE.MeshStandardMaterial({ color: 0xcbd5e1, metalness: 0.95, roughness: 0.1 });
    const hazardYellowMat = new THREE.MeshBasicMaterial({ color: 0xf59e0b });
    const hazardBlackMat = new THREE.MeshBasicMaterial({ color: 0x020617 });
    const ledGlowMat = new THREE.MeshBasicMaterial({ color: 0x00f0ff }); // Cyan LED strip
    ...
    [-railXOffset, railXOffset].forEach((rx) => {
      ...
      for (let p = 0; p <= numPosts; p++) {
        ...
        const post = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.1, 12), steelMat);
        post.position.set(rx, 0.95, pz);
        group.add(post);

        // Flange Mounting Baseplate
        const baseplate = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.06, 0.2), darkSteelMat);
        ...
        for (let b = 0; b < 4; b++) {
          ...
          const bolt = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.08, 8), chromeMat);
    ...
    for (let b = 0; b <= numBeams; b++) {
      ...
      for (let t = -1; t <= 1; t += 2) {
        const brace = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 1.2, 8), steelMat);
    ...
    [-halfLen, halfLen].forEach((endZ) => {
      ...
      for (let s = -width / 2 + 0.4; s < width / 2; s += 0.8) {
        const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.09, 0.62), hazardBlackMat);
  ...
  public static createUltraDetailedStagePlatform(
    ...
    // Materials
    const steelMat = floorMat || new THREE.MeshStandardMaterial({ color: 0x1e293b, metalness: 0.85, roughness: 0.25 });
    const darkSteelMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, metalness: 0.9, roughness: 0.2 });
    const plateMat = new THREE.MeshStandardMaterial({ color: 0x334155, metalness: 0.8, roughness: 0.3 });
    const yellowHazardMat = new THREE.MeshBasicMaterial({ color: 0xf59e0b });
    const blackHazardMat = new THREE.MeshBasicMaterial({ color: 0x020617 });
    ...
    for (let c = 0; c < plateCols; c++) {
      for (let r = 0; r < plateRows; r++) {
        ...
        [
          ...
          const rivet = new THREE.Mesh(new THREE.CylinderGeometry(0.02, 0.02, 0.05, 8), chromeMat);
    ...
    [-depth / 2 + 0.15, depth / 2 - 0.15].forEach((zEdge) => {
      ...
      for (let s = -width / 2 + 0.5; s < width / 2 - 0.5; s += 1.0) {
        const stripe = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.06, 0.32), blackHazardMat);
    ...
    [
      ...
      // Flange Base Collar
      const collar = new THREE.Mesh(new THREE.CylinderGeometry(0.6, 0.7, 0.2, 12), darkSteelMat);
      collar.position.set(cx, -legH - 0.3, cz);
      group.add(collar);

      // Corner Amber Warning Beacon Light
      const beacon = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.2, 12), cyanLedMat);
  ...
  public static createUltraDetailedTacticalCoverCrate(
    ...
    // Materials
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, roughness: 0.4, metalness: 0.75 });
    const frameMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, roughness: 0.3, metalness: 0.85 });
    const bumperMat = new THREE.MeshStandardMaterial({ color: 0x334155, roughness: 0.5, metalness: 0.6 });
    const chromeMat = new THREE.MeshStandardMaterial({ color: 0xcbd5e1, roughness: 0.15, metalness: 0.95 });
    const warningRedMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });
    const warningYellowMat = new THREE.MeshBasicMaterial({ color: 0xf59e0b });
    const ledGlowMat = new THREE.MeshBasicMaterial({ color: 0x22c55e }); // Glowing green active LED
    ...
    [-halfW, halfW].forEach((x) => {
      [-halfH, halfH].forEach((y) => {
        [-halfD, halfD].forEach((z) => {
          const bumper = new THREE.Mesh(new THREE.BoxGeometry(0.3, 0.3, 0.3), bumperMat);
          bumper.position.set(
            x + (x > 0 ? -0.1 : 0.1),
            y + (y > 0 ? -0.1 : 0.1),
            z + (z > 0 ? -0.1 : 0.1)
          ...
          // Corner bolt
          const bolt = new THREE.Mesh(new THREE.SphereGeometry(0.04, 8, 8), chromeMat);
          bolt.position.set(
            x + (x > 0 ? 0.02 : -0.02),
            y + (y > 0 ? 0.02 : -0.02),
            z + (z > 0 ? 0.02 : -0.02)
    ...
    [-halfD - 0.02, halfD + 0.02].forEach((zPos) => {
      const latch = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.25, 0.08), chromeMat);
      latch.position.set(0, 0.2 * height, zPos);
      group.add(latch);

      const handleBar = new THREE.Mesh(new THREE.TorusGeometry(0.12, 0.025, 8, 12, Math.PI), chromeMat);
    ...
    const statusLed = new THREE.Mesh(new THREE.SphereGeometry(0.06, 12, 12), ledGlowMat);
  ...
  public static createUltraDetailedCommandHubPedestal(): THREE.Group {
    ...
    const baseGeo = new THREE.CylinderGeometry(2.8, 3.2, 1.2, 8);
    ...
    const rimGeo = new THREE.CylinderGeometry(3.1, 3.3, 0.2, 8);
    const rimMesh = new THREE.Mesh(rimGeo, steelMat);
    rimMesh.position.y = 1.1;
    group.add(rimMesh);

    // 3. Glowing Cyan LED Perimeter Ring
    const ledRingGeo = new THREE.TorusGeometry(2.9, 0.06, 8, 32);
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const rivet = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 0.06, 8), chromeMat);
    ...
    for (let c = 0; c < 4; c++) {
      ...
      const pipe = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 1.2, 8), chromeMat);
